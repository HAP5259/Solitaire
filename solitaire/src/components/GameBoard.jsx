...
export default function GameBoard() {
    ...
    // Reserve sidebar width when debug is open so scale accounts for it
    const reservedRight = isDebugOpen ? 384 : 0; // w-96 = 24rem = 384px
    const scale = useScaleToFit(boardRef, { padding: 24, reservedRight, reservedBottom: 0, maxScale: 1 });
    ...
    return (
        <div className="w-full flex">
            <div className={`flex-1 transition-all duration-300 ${isDebugOpen ? 'mr-96' : 'mr-0'} max-w-7xl mx-auto`}>
                <div ref={boardRef} style={{ transform: `scale(${scale})`, transformOrigin: 'top center' }} className="bg-white rounded-xl shadow-xl p-6 inline-block">
            ...
            <DebugMenu onOpenChange={setIsDebugOpen} />
        </div>
    );
}